...
object ParticipantTableDrawer {
    ...
    fun draw(
        ...
        PdfDrawingUtils.drawTextCentered(
            cs, semiBoldFont, DocumentLayout.fontSizeSubTitle,
            contentW + x * 2,
    ...
    private fun rowValues(seq: Int, p: ParticipantInfo?): List<String> =
        if (p != null)
            listOf(seq.toString(), p.grade.toString(), p.classNum.toString(),
                p.num.toString(), p.name, p.recognizedHours.toString())
        else
            listOf(seq.toString(), "", "", "", "", "")
    ...
    private fun drawCells(
        ...
        values.forEachIndexed { i, v ->
            ...
            if (isHeader || i == 0) PdfDrawingUtils.fillRect(
                cs, cx, topY - DocumentLayout.participantRowH, cw, DocumentLayout.participantRowH, DocumentLayout.labelBgColor
            )
